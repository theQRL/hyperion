...
[[ $binary_type == native || $binary_type == hypcjs ]] || { >&2 echo "Invalid binary type: ${binary_type}"; exit 1; }
...
rm ./*_bytecode_too_large_*.hyp ./*_combined_too_large_*.hyp
...
if [[ $binary_type == native ]]; then
    ...
    echo "Installing hypc-js"
    git clone --depth 1 https://github.com/theQRL/hypc-js.git hypc-js
    cp "$binary_path" hypc-js/hypjson.js
    ...
    ./prepare_report.js \
        --preset "$preset" \
        *.hyp > "../bytecode-report-${label}-${preset}.txt"
fi
