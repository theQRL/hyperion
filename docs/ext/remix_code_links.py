...
def remix_code_url(source_code, language, hyperion_version):
    ...
    return f"https://remix.ethereum.org/?#language={language}&version={hyperion_version}&code={base64_encoded_source}"
...
def insert_remix_link(app, doctree, hyperion_version):
    ...
    for literal_block_node in doctree.traverse(docutils.nodes.literal_block):
        ...
        if language not in ['hyperion', 'yul']:
            continue

        text_nodes = list(literal_block_node.traverse(docutils.nodes.Text))
        assert len(text_nodes) == 1

        remix_url = remix_code_url(text_nodes[0], language, hyperion_version)
...
def setup(app):
    ...
    hyperion_version = app.config._raw_config['version']  # pylint: disable=protected-access

    app.connect(
        'doctree-resolved',
        lambda app, doctree, docname: insert_remix_link(app, doctree, hyperion_version)
    )
