...
One can use ``hypc --ir-optimized --optimize`` to produce an
optimized Yul IR for a Hyperion source. Similarly, one can use ``hypc --strict-assembly --optimize``
...
Benefits of Optimizing Hyperion Code
====================================
...
Since Hyperion version 0.8.2, there is another optimizer step that replaces certain
jumps to blocks containing "simple" instructions ending with a "jump" by a copy of these instructions.
This corresponds to inlining of simple, small Hyperion or Yul functions. In particular, the sequence
...
call to an internal Hyperion function:
...
.. code-block:: bash
    ...
    hypc --optimize --ir-optimized --yul-optimizations 'dhfoD[xarrscLMcCTU]uljmul:fDnTOcmu'
...
will not change and the Common Subexpression Eliminator reuses exactly the same
