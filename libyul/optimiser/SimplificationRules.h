...
public:
	...
	using Rule = qrvmasm::SimplificationRule<Pattern>;

	explicit SimplificationRules(std::optional<langutil::QRVMVersion> _qrvmVersion = std::nullopt);
	...
	static std::optional<std::pair<qrvmasm::Instruction, std::vector<Expression> const*>>
...
private:
	...
	std::vector<qrvmasm::SimplificationRule<Pattern>> m_rules[256];
};
...
public:
	...
	Expression toExpression(std::shared_ptr<DebugData const> const& _debugData, langutil::QRVMVersion _qrvmVersion) const;
