...
/// are bound by the minimum and the maximum values permitted by the Hyperion type int256.
library PRBMathSD59x18 {
    ...
    function ceil(int256 x) internal pure returns (int256 result) {
        ...
        unchecked {
            ...
            if (remainder == 0) {
                ...
                // Hyperion uses C fmod style, which returns a modulus with the same sign as x.
                result = x - remainder;
                if (x > 0) {
                    result += SCALE;
                }
    ...
    function gm(int256 x, int256 y) internal pure returns (int256 result) {
        ...
        unchecked {
            // Checking for overflow this way is faster than letting Hyperion do it.
            int256 xy = x * y;
            require(xy / x == y);
